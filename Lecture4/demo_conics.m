...
% using the technique shown in "Multiple View Geometry", page 9. this is 
% equivalent to requiring that the conic passes trhough all the six points
...
% (which is symmetrical) Let's start with a first solution, the first vector of the nullspace
cc = N(:, 1);
...
% coordinates is l the line tangent to C at the line x is defined as l = C*x
...
% in order to draw it we find its angle, then plot a line of length 200 around it.
% In order to write less lines of code, I'm doing a bit of hacks with matrices
% and vectors here: theta will be a column vector of angles, and with a
% single plot command I'll plot all the lines. Check out the documentation
% of plot to understand what's happening.
...
% computed previously actually satisfy the relation (up to numeric precision issues).
Cstar=inv(C);
l(:,1)'*Cstar*l(:,1)
l(:,2)'*Cstar*l(:,2)
l(:,3)'*Cstar*l(:,3)
